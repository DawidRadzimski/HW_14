...
from src.repository.contacts import (
    ...
    remove_contact,
    update_contact,
    get_contacts_birthday_for_next_week,
)
...
class TestContacts(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_find_contact_found(self):
        contacts = [Contact(name="Anna"), Contact(email="annakowalska@op.pl"), Contact(lastname="Annamanna")]
        word = "Anna"
        self.session.query().filter(and_(or_(Contact.name.ilike(word), Contact.lastname.ilike(word), Contact.email.ilike(word)))).offset().limit().all.return_value = contacts
        result = await find_contact(keyword="Anna", skip=0, limit=10, user=self.user, db=self.session)
        self.assertEqual(result, contacts)

    async def test_find_contact_not_found(self):
        word = "Anna"
        self.session.query().filter(and_(or_(Contact.name.ilike(word), Contact.lastname.ilike(word), Contact.email.ilike(word)))).offset().limit().all.return_value = None
        result = await find_contact(keyword="Anna", skip=0, limit=10, user=self.user, db=self.session)
        self.assertIsNone(result)

    async def test_get_contacts_birthday_for_next_week_found(self):
        ...
        contacts = [Contact(birthday = birthday1), Contact(birthday = birthday2), Contact(birthday = birthday3)]
    ...
    async def test_get_contacts_birthday_for_next_week_notfound(self):
        today = datetime.now().date()
        next_week = today + timedelta(days=7)
        self.session.query().filter(and_(
        or_(
        ...
        result = await get_contacts_birthday_for_next_week(skip=0, limit=10, user=self.user, db=self.session)
        self.assertIsNone(result)
